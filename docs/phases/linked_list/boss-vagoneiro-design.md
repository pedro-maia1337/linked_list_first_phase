...
- ✅ **Reorganização em compartilhado × fase.** Refatoração só de estrutura, sem mudança de comportamento (render idêntico pixel a pixel ao anterior). O Player, o HUD, os efeitos genéricos, a física e o debug foram para `lib/shared/` + `assets/shared/`; tudo o que é do Vagoneiro foi para `lib/phases/linked_list/` + `assets/phases/linked_list/`. Entraram o contrato `PhaseDefinition`/`PhaseGame` e o registro de fases. Este documento passou a morar em `docs/phases/linked_list/`. Guia: `docs/ESTRUTURA.md`; mapa de caminhos no fim deste documento.
...
## Apêndice — Mapa de caminhos após a reorganização compartilhado × fase

As seções acima citam os caminhos da época de cada módulo. A estrutura atual está descrita em `docs/ESTRUTURA.md`. Correspondência:

| Antes | Agora |
|---|---|
| `lib/game/vagoneiro_arena_game.dart` | `lib/phases/linked_list/vagoneiro_arena_game.dart` (estende `PhaseGame`) |
| `lib/game/boss/*`, `wagon/*`, `track/*`, `config/*` | `lib/phases/linked_list/{boss,wagon,track,config}/*` |
| `lib/game/fx/ambient_light.dart`, `boss_dock.dart`, `boss_rim_light.dart`, `wagon_fx_observer.dart` | `lib/phases/linked_list/fx/` |
| `lib/game/fx/combat_fx.dart` | `SlotHighlight` → `lib/phases/linked_list/fx/slot_highlight.dart`; `CombatFlash` → `lib/shared/fx/combat_flash.dart` |
| `lib/game/fx/sprite_burst.dart` | `ParticleSheets`/`SpriteBurst`/`SpriteLoop`/poeira → `lib/shared/fx/sprite_burst.dart`; faísca e fumaça → `lib/phases/linked_list/fx/linked_list_particles.dart` (`sparkEmberBurst`, `sparkEmberLoop`, `smokeWispBurst`) |
| `lib/game/fx/fx_config.dart` | genéricas → `lib/shared/fx/fx_config.dart`; da fase → `lib/phases/linked_list/fx/linked_list_fx_config.dart` (mesmos nomes e valores; `wagonFallShake*` e `empurrao*` viraram aliases de `cameraShake*` e `playerShove*`) |
| `lib/game/fx/{camera_director,contact_shadow,game_curves,glow_light,parallax_backdrop}.dart` | `lib/shared/fx/` (`SkyFill`/`ParallaxLayer` agora recebem a cor do céu por parâmetro) |
| `lib/game/player/*` | `lib/shared/player/*` (`arena_wall.dart` → `lib/shared/physics/`) |
| `lib/game/hud/combat_hud.dart` | `lib/shared/hud/combat_hud.dart` (nome, HP máximo, limiares e rótulo de fase do boss vêm por parâmetro) |
| `lib/game/components/*` | `lib/shared/debug/*` (a legenda veio para `linked_list_debug_keys.dart`) |
| `playerMaxHp`, `playerInvulnerabilityDuration`, `playerStagger*`, `playerAttackDamage` em `boss_combat.dart` | `lib/shared/player/player_combat_config.dart` |
| `Player` pousava só em `Vagao` e recebia `LinearTrack` | pousa em qualquer `PlayerPlatform` (`Vagao` implementa); o índice de slot vem do callback `platformIndexOf` que a fase fornece |
| `docs/slots_config.json` | `assets/phases/linked_list/level/slots_config.json` |
| `assets/{background,boss,wagons,tilesets}/` | `assets/phases/linked_list/{...}/` |
| `assets/effects/` | `contact-shadow`, `glow-warm-soft`, `particles/dust-poof` → `assets/shared/effects/`; o resto → `assets/phases/linked_list/effects/` |
| `assets/player/` | `assets/shared/player/` |
| `test/*.dart` | `test/phases/linked_list/*` e `test/shared/player_upgrade_test.dart` (+ `test/shared/phase_structure_test.dart`, novo) |
| teclas `N`/`S` e `J` no `DebugHelpOverlay` | legenda em `linkedListDebugLegend` |
