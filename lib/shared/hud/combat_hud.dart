...
import '../player/player_combat_config.dart' as player_config;
...
///
/// Shared by every phase: the boss's name, max HP, phase thresholds and
/// phase label are supplied by the phase (the linked_list phase passes
/// 'O VAGONEIRO', 12 HP, the 0.66/0.33 bands and `BossPhase.label`).
class CombatHud extends PositionComponent {
  ...
  /// Upper-case name drawn under the boss bar.
  final String bossName;
  final int bossMaxHp;

  /// HP fractions (0..1) where a new boss phase begins, drawn as ticks.
  final List<double> phaseThresholds;

  /// The current phase's human-readable label.
  final String Function() phaseLabel;

  /// Label under the player's HP pips.
  ...
  CombatHud({
    ...
    this.playerLabel = 'MAQUINISTA',
    this.playerMaxHp = player_config.playerMaxHp,
  ...
  void _renderBossBar(Canvas canvas) {
    ...
    for (final threshold in phaseThresholds) {
    ...
    _labelPaint.render(
      canvas,
      '$bossName   ${bossHp()}/$bossMaxHp   ${phaseLabel()}$status',
