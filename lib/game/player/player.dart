...
class Player extends PositionComponent with CollisionCallbacks {
  ...
  /// Extension hook invoked every time [onPlayerDeath] fires; the arena uses
  /// it to charge [quedaDamage].
  final void Function()? onDeath;

  /// The arena's slot geometry, read-only.
  final LinearTrack track;

  /// Fired when the wagon under the player stops being solid.
  ...
  /// The [Vagao] currently supporting the player, if any.
  ...
  void onCollision(Set<Vector2> intersectionPoints, PositionComponent other) {
    ...
    // `other` is the hitbox's parent entity — the Vagao itself.
    if (other is! Vagao) {
      return;
    }
