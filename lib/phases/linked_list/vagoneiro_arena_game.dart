...
import '../../shared/debug/debug_help_overlay.dart';
import '../../shared/debug/debug_marker.dart';
import '../../shared/debug/height_ruler.dart';
import '../../shared/fx/camera_director.dart';
import '../../shared/fx/contact_shadow.dart';
import '../../shared/fx/fx_config.dart';
import '../../shared/fx/glow_light.dart';
import '../../shared/fx/parallax_backdrop.dart';
import '../../shared/fx/sprite_burst.dart';
import '../../shared/hud/combat_hud.dart';
import '../../shared/phases/phase_definition.dart';
import '../../shared/phases/phase_game.dart';
import '../../shared/physics/arena_wall.dart';
...
import 'fx/linked_list_particles.dart';
import 'fx/wagon_fx_observer.dart';
import 'linked_list_debug_keys.dart';
import 'linked_list_phase.dart';
...
class VagoneiroArenaGame extends PhaseGame
    with KeyboardEvents, HasCollisionDetection {
  @override
  PhaseDefinition get definition => linkedListPhase;

  /// Above slot 0, so gravity drops the player straight onto its wagon.
  @override
  Vector2 get spawnPoint => _spawnPoint.clone();
  late final Vector2 _spawnPoint;

  /// Victory condition: the Vagoneiro is defeated.
  @override
  bool get isCompleted => boss.isDefeated;
  ...
  Future<void> onLoad() async {
    ...
    combatHud = CombatHud(
      ...
      bossMaxHp: bossMaxHp,
      phaseThresholds: const [bossPhase1LowerBound, bossPhase2LowerBound],
      phaseLabel: () => boss.phase.label,
    );
    ...
    camera.viewport.add(
      DebugHelpOverlay(position: Vector2(16, 16), lines: linkedListDebugLegend),
    );
  ...
  /// The player's logical position in the list: the index of the slot
  /// holding [platform], for linked wagons only (a wagon outside the list
  /// has no index). Handed to the shared [Player] as `platformIndexOf`.
  int? _slotIndexOf(PlayerPlatform platform) {
    if (platform is! Vagao || !platform.linked) {
      return null;
    }
    for (final slot in track.slots) {
      if (identical(slot.vagao, platform)) {
        return slot.index;
      }
    }
    return null;
  ...
  Future<void> _addParallaxBackdrop() async {
    ...
    await world.add(SkyFill(camera: camera, color: parallaxSkyFillColor));
    ...
    await world.add(
      await ParallaxLayer.load(
        ...
        skyFillColor: parallaxSkyFillColor,
...
class _BossSmokeEmitter extends Component {
  ...
  Future<void> _emit() async {
    ...
    final wisp = await smokeWispBurst(
      origin,
